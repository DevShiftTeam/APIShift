      ...
      items: [
        {
          name: "wait",
          id: 1,
          rect: { x: 20, y: 0, width: 0, height: 0 },
          is_relation: false,
          data: {},
        },
        {
          name: "haha",
          id: 2,
          rect: { x: 220, y: 200, width: 0, height: 0 },
          is_relation: false,
          data: {},
        },
        {
          name: "rela",
          id: 3,
          rect: { x: 120, y: 50, width: 0, height: 0 },
          is_relation: true,
          data: {
            from: { id: 1, type: "i" },
            to: { id: 2, type: "i" },
  ...
  methods: {
    ...
    pointer_down(event) {
      ...
      if (this.tap_counter === 2) {
        ...
        window.temp_pointer = {
          x:
            (window.init_pointer_first.x + window.init_pointer_second.x) / 2 -
            window.graph_position.x,
          y:
            (window.init_pointer_first.y + window.init_pointer_second.y) / 2 -
            window.graph_position.y,
        };
      ...
      if (cursor_state.type === "create") {
        ...
        let center_rect = document
          .getElementById("graph_center")
          .getBoundingClientRect();
        let mouse = {
          x: window.init_pointer.x - graph_position.x,
          y: window.init_pointer.y - graph_position.y,
        };
        let differential = {
          x: center_rect.x - graph_position.x,
          y: center_rect.y - graph_position.y,
        };
        let t_mouse = {
          x: (mouse.x - differential.x) / this.scale,
          y: (mouse.y - differential.y) / this.scale,
        };
    ...
    pointer_move(event) {
      this.camera.x =
        this.init_camera.x + event.clientX - window.init_pointer.x;
      this.camera.y =
        this.init_camera.y + event.clientY - window.init_pointer.y;
    },
    pointer_scale(event) {
      this.update_graph_position();
      let rect = document
        ...
        .getBoundingClientRect();
      // Calculate center
      window.init_pointer.x =
        (window.init_pointer_first.x + window.init_pointer_second.x) / 2 -
        window.graph_position.x;
      window.init_pointer.y =
        (window.init_pointer_first.y + window.init_pointer_second.y) / 2 -
        window.graph_position.y;
      ...
      if (
        Math.abs(window.init_pointer_first.y - event.clientY) +
          Math.abs(window.init_pointer_first.x - event.clientX) <
        Math.abs(window.init_pointer_second.y - event.clientY) +
          Math.abs(window.init_pointer_second.x - event.clientX)
      ) {
      ...
      // Move Camera
      this.camera.x +=
        (window.init_pointer.x - mid.x) * (1 - change) +
        window.init_pointer.x -
        window.temp_pointer.x;
      this.camera.y +=
        (window.init_pointer.y - mid.y) * (1 - change) +
        window.init_pointer.y -
        window.temp_pointer.y;
    ...
    key_down(event) {
      if (event.code === "KeyG") {
        ...
        graph_view.create_element_on_runtime("group", {
          name: "Group",
          rect: { x: 0, y: 0, height: 0, width: 0 },
          data: { contained_elements },
        });
      }
    },
    pan_by(dx, dy) {
     ...
     */
    create_line(
      src_info = { type: "", from_id: 0 },
      dest_info = { type: "", to_id: 0 },
      settings = {}
    ) {
    ...
    delete_line(line_uid) {
      ...
      setTimeout(
        () =>
          (graph_view.lines = graph_view.lines.filter(
            (line) => line.line_uid !== line_uid
          )),
        0
      );
    },
    relation_builder(relate_from = {}, relate_to = {}, relate_type) {
      ...
      if (Object.values(this.relation_factory).every((v) => v)) {
        let from = {
          type: this.relation_factory.from.type,
          id: this.relation_factory.from.id,
        };
        let to = {
          type: this.relation_factory.from.type,
          id: this.relation_factory.from.id,
        ...
        let rect = {
          x:
            (this.relation_factory.from.rect.x +
              this.relation_factory.from.rect.x) /
            2,
          y:
            (this.relation_factory.from.rect.y +
              this.relation_factory.from.rect.y) /
            2,
          width: 0,
          height: 0,
        ...
        this.create_element_on_runtime("item", {
          name: "relation",
          rect,
          data: { from, to, type },
        });
    ...
    create_element_on_runtime(type, properties = {}) {
      ...
      if (type === "enum-type") {
        let enum_type_id =
          Math.max(...graph_view.enum_types.map((t) => t.id), 0) + 1;
        console.log();
...
<template ref="graphview">
  ...
  >
    ...
    <div ref="gv_menu" id="gv_menu"></div>
    <component ref="s_box" :is="selection_comp"> </component>
  </div>
</template>
