<script>
    ...
    module.exports = {
        ...
        beforeRouteEnter (to, from, next) {
            if(APIShift.logged_in) next("/main");
            next();
        },
        created () { },
        updated () {
        ...
        methods: {
            ...
            findIncorrectField: function (update = true) {
                ...
                for(field_id in this.credentials.fields)
                {
                    ...
                    if(field.regex !== undefined && !RegExp(field.regex).test(field.data)) {
                        if(update) APIShift.API.notify("Invalid format in " + field.label + ": " + field.format, "warning");
                        return field_id; // return field who is incorrect or empty
                    }
                }
            ...
            focusOnIncorrectField: function (handler = this) {
                ...
                if(field_id != -1) $('#' + handler.credentials.fields[field_id].name).focus();
            ...
            login: function() {
                ...
                for(var field_id in this.credentials.fields)
                    to_send[this.credentials.fields[field_id].name] = this.credentials.fields[field_id].data;
