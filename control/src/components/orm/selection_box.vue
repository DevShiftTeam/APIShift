...
module.exports = {
  ...
  methods: {
    start_select(event) {
      this.init_pointer = Object.assign(
        {},
    ...
    on_select(event) {
      ...
      graph_view.items.forEach((item) => {
        if (graph_view.hittest("s_box", "i" + item.id)) {
          if (!graph_view.$refs["i" + item.id].group_owner)
      ...
      graph_view.groups.forEach((group) => {
        if (graph_view.hittest("s_box", "g" + group.id)) {
          if (!graph_view.$refs["g" + group.id].group_owner)
            graph_view.$refs["g" + group.id].selected = true;
        } else {
    ...
    on_scroll() {
      ...
      let mouse = {
        x: this.last_event.pageX - window.graph_position.x,
        y: this.last_event.pageY - window.graph_position.y,
      };
      if (mouse.x < 20) {
        this.ds.x += 5;
    ...
    set_rect(event) {
      ...
      if (event.clientX > this.init_pointer.x + this.ds.x) {
        ...
        calcLeft =
          this.init_pointer.x - graph_position.x + this.ds.x - this.width;
      }
      ...
      if (event.clientY > this.init_pointer.y + this.ds.y) {
        ...
        calcTop =
          this.init_pointer.y - graph_position.y + this.ds.y - this.height;
      }
...
<template>
  <div class="s_box" color="#8789ff" :style="transformation"></div>
</template>
